...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Standby Current", "120uA, 75uA", {"current 1": 120e-6, "current 2": 75e-6}),
        ...
        ("Supply Current (Write)", "280uA", {"current": 280e-6}),
...
def test_output_identifiers(capsys):
    values = normalized_values("Output", "LVCMOS, HCSL", capsys)
...
def test_output_polarity_identifiers(capsys):
    values = normalized_values("Output Polarity", "Positive electrode;Negative electrode", capsys)

    assert values["polarity 1"] == ["Positive", "identifier"]
    assert values["polarity 2"] == ["Negative", "identifier"]

    values = normalized_values("Output Polarity", "Positive and Negative Poles", capsys)
...
def test_load_type_identifiers(capsys):
    ...
    assert values["load type 1"] == ["MOSFET", "identifier"]
    assert values["load type 2"] == ["IGBT", "identifier"]
    assert values["load type 3"] == ["SiC", "identifier"]
    assert values["load type 4"] == ["GaN", "identifier"]

    values = normalized_values("Load Type", "AC,DC", capsys)

    assert values["load type 1"] == ["AC", "identifier"]
    assert values["load type 2"] == ["DC", "identifier"]

    values = normalized_values("Load Type", "-", capsys)

    assert values["load type 1"] == ["Unspecified", "identifier"]
...
def test_luminous_color_identifiers(capsys):
    values = normalized_values("Luminous Color", "自然白色", capsys)

    assert values["color 1"] == ["Natural White", "identifier"]


def test_lens_color_identifiers(capsys):
    values = normalized_values("Lens Color", "Frosted Red Lens, Frosted Green Lens", capsys)

    assert values["lens color 1"] == ["Frosted Red Lens", "identifier"]
...
def test_plating_and_product_description_dimensions(capsys):
    ...
    values = normalized_values("Surface Treatment", 'Nickel-Plated 50u, Semi-Gold Plated 1+1u', capsys)
    assert_quantity(values["nickel thickness min"], 50 * 0.0254e-6, "length")
    assert_quantity(values["nickel thickness max"], 50 * 0.0254e-6, "length")
    assert_quantity(values["semi gold thickness min"], 2 * 0.0254e-6, "length")
    assert_quantity(values["semi gold thickness max"], 2 * 0.0254e-6, "length")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Power Rating", "0.6W", 0.6),
        ("Maximum Switching Power", "100W", 100.0),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Overall Length", "180mm, 300mm", {"length 1": 0.18, "length 2": 0.3}),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Propagation Delay", "19ns, 23ns", {"time 1": 19e-9, "time 2": 23e-9}),
