    ...
    try:
        if key in larr(["Resistance", "Resistance in Ohms @ 25°C", "DC Resistance"]):
            ...
            value = attributes.countAttribute(value)
                ...
                "Reverse Transfer Capacitance (Cres)", "Con"]):
            ...
            value = attributes.stringAttribute(value) if multiScalarValue(value) else attributes.inductanceAttribute(value)
                ...
                "Height - Seated (Max)", "Length of Mating Pin", "Operating Height", "L"]):
            if key in larr(["Insulation Od", "Interface Length/Height", "Interface Diameter"]) and isinstance(value, str) and "," in value:
            ...
            if isinstance(value, str) and re.search(r"(?:bit/s|bps)\s*$", value, flags=re.IGNORECASE):
                ...
                elif key in ["sampling rate", "frequency - switching", "clock frequency", "transition frequency (f t)", "sampling frequency", "center frequency"] and isinstance(value, str) and ("," in value or ";" in value) and "~" in value:
                    value = attributes.frequencyRangeListAttribute(value)
                elif key in ["sampling rate", "frequency - switching", "clock frequency", "transition frequency (f t)", "sampling frequency", "center frequency"] and isinstance(value, str) and ("," in value or ";" in value):
            ...
            value = attributes.stringAttribute(value) if compoundValue(value) else attributes.inductanceAtFrequency(value)
                ...
                "Reset Timeout", "Settling Time", "Response Time (Tr)", "Td(Off)"]):
            if compoundValue(value) and "@" not in value:
                if key in larr(["Propagation Delay (TPD)", "Reset Timeout", "Settling Time", "Response Time (Tr)", "Td(Off)"]) and isinstance(value, str) and "," in value:
